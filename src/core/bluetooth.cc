...
using namespace ssc::core::headers;
using namespace ssc::core::utils;
using namespace ssc::core;

using ssc::core::data::CoreData;

namespace ssc::core::bluetooth {
  CoreBluetoothInternals::CoreBluetoothInternals (CoreBluetooth* coreBluetooth) {
    this->coreBluetooth = coreBluetooth;
    #if defined(__APPLE__)
      this->coreBluetoothController = [CoreBluetoothController new];
      this->coreBluetoothController.internals = this;
    #endif
  }

  CoreBluetoothInternals::~CoreBluetoothInternals () {
    #if defined(__APPLE__)
      if (this->coreBluetoothController != nullptr) {
        #if !__has_feature(objc_arc)
          [this->coreBluetoothController release];
        #endif
        this->coreBluetoothController = nullptr;
      }
    #endif
  }
  ...
  void CoreBluetooth::startScanning () {
      ...
      if (
        this->internals != nullptr &&
        this->internals->coreBluetoothController != nullptr
      ) {
        [this->internals->coreBluetoothController startScanning];
      }
    #endif
  }

  void CoreBluetooth::publishCharacteristic (
    const String &seq,
    char *bytes,
    size_t size,
    ...
    const String &characteristicId
      ...
      auto controller = this->internals->coreBluetoothController;
  ...
  void CoreBluetooth::subscribeCharacteristic (
    const String &seq,
    const String &serviceId,
    const String &characteristicId
  ) {
  ...
  void CoreBluetooth::startService (
    const String &seq,
    const String &serviceId
  ) {
    auto json = JSON::Object::Entries {
...
- (void) peripheralManagerDidUpdateState: (CBPeripheralManager*) peripheral {
  ...
  self.internals->coreBluetooth->emit("bluetooth", JSON::Object(json).str());
}
                           ...
                           error: (NSError*) error
{
  ...
  self.internals->coreBluetooth->send("-1", JSON::Object(json).str(), data);
}
