...
namespace ssc::runtime {
  FS::FS (Runtime* runtime)
    : descriptorManager(runtime->descriptorManager),
      timers(runtime->timers),
      loop(runtime->loops.main)
  {}

  FSRequestContext::FSRequestContext (Descriptor *desc)
    : FSRequestContext(desc, "", nullptr)
  {}

  FSRequestContext::FSRequestContext (String seq, Callback callback)
    : FSRequestContext(nullptr, seq, callback)
  {}

  FSRequestContext::FSRequestContext (Descriptor *desc, String seq, Callback callback) {
    this->id = rand64();
  ...
  FSRequestContext::~FSRequestContext () {
    uv_fs_req_cleanup(&this->req);
  }

  void FSRequestContext::setBuffer (int index, size_t len, char *base) {
    this->iov[index].base = base;
    this->iov[index].len = len;
  }

  void FSRequestContext::freeBuffer (int index) {
...
char* FSRequestContext::getBuffer (int index) {
  return this->iov[index].base;
}

size_t FSRequestContext::getBufferSize (int index) {
  return this->iov[index].len;
}

  Descriptor* DescriptorManager::create (uint64_t id) {
  ...
  Descriptor* DescriptorManager::get (uint64_t id) {
    Lock lock(this->mutex);
  ...
  void DescriptorManager::remove (uint64_t id) {
    Lock lock(this->mutex);
    if (descriptors.find(id) != descriptors.end()) {
  ...
  bool DescriptorManager::has (uint64_t id) {
    Lock lock(this->mutex);
    return descriptors.find(id) != descriptors.end();
  }

  size_t DescriptorManager::count () {
    Lock lock(this->mutex);
    return this->descriptors.size();
  }

  DescriptorManager::DescriptorManager (Runtime* runtime)
    : timers(runtime->timers),
      loop(runtime->loops.main)
  {
  ...
  void FS::close (
    ...
    this->loop.dispatch([=, this]() {
      ...
      auto ctx = new FSRequestContext(desc, seq, callback);
      auto req = &ctx->req;
  ...
  void FS::read (
    ...
    this->loop.dispatch([=, this]() {
      ...
      auto err = uv_fs_read(loop.get(), req, desc->fd, ctx->iov, 1, offset, [](uv_fs_t* req) {
        auto ctx = static_cast<FSRequestContext*>(req->data);
        auto desc = ctx->desc;
        auto json = JSON::Object {};
        Data data = {0};
