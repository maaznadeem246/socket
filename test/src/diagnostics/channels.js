...
test('diagnostics - channels - shared', async (t) => {
  const channel = await diagnostics.channels.shared()
  const client = dgram.createSocket('udp4')
  const { port, address } = channel.socket.address()

  await new Promise((resolve) => {
    client.connect(port, address, (err, info) => {
      if (err) return t.ifError(err)
      client.send('hello world', 0, 11, port, address)
      resolve()
    })
  })
})
